...
/*   Updated: 2021/03/18 21:09:10 by lsuardi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <list.h>

void *	two_dimensional_dup_f(const void * data)
{
	t_list *	src;
	t_list *	new_member;
	t_list *	new_data;
	...
	if (!(new_data->data = default_dup_f(src->data)))
	...
	new_data->next = NULL;
	return ((void*)new_member);
}

void *	two_dimensional_share_dup_f(const void * data)
{
	t_list *	src;
	t_list *	new_member;
	t_list *	new_data;
...
void *	three_dimensional_dup_f(const void * data)
{
	t_list *	src;
	t_list *	new_member;
	t_list *	new_data;
	...
	if (!(new_data = malloc(sizeof(t_list))))
		return (lst_clear(&new_member, NO_FREE));
	new_member->data = (void*)new_data;
	if (!(new_data->data = two_dimensional_dup_f(src->data)))
...
void *	three_dimensional_share_dup_f(const void * data)
{
	t_list *	src;
	t_list *	new_member;
	t_list *	new_data;
